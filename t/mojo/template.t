...
$output = $mt->render_file_has_hook($c,  $file, 3 );
like $output, qr/23\nHello World!/, 'file';

# Exception in file
$mt     = Mojo::Template->new;
$file   = catfile dirname(__FILE__), 'templates', 'exception.mt';
$output = $mt->render_file_has_hook($c, $file);
...
$mt     = Mojo::Template->new;
$output = $mt->name('"foo.mt" from DATA section')->render_file_has_hook($c, $file);
isa_ok $output, 'Mojo::Exception', 'right exception';
...
$file = catfile dirname(__FILE__), 'templates', 'utf8_exception.mt';
ok !eval { $mt->render_file_has_hook($c, $file) }, 'file not rendered';
