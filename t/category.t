...
subtest run_tests => sub {
    ...
    my $nodes   = $schema->resultset('Category');
    my $root    = $nodes->create( { name => 'root' } );
    my @parents = (
        1,  1,  3,  4,  4,  3,  3,  8,  8,  10, 10, 8,  8,  3,  3,  16, 3,  3,  1,  20,
        1,  22, 22, 24, 24, 22, 27, 27, 29, 29, 27, 32, 32, 34, 34, 36, 34, 38, 38, 40,
        40, 42, 42, 44, 44, 46, 44, 44, 49, 44, 51, 51, 53, 51, 55, 55, 57, 57, 55, 60,
        55, 62, 55, 64, 64, 55, 67, 67, 55, 70, 70, 55, 55, 51, 51, 76, 76, 78, 78, 76
    );
    ...
    ok( ( ( $nodes->find(22)->parents->all() )[0]->id() == $nodes->find(22)->parent->id() ),
        'node 22 parent matches parents' );
    ...
    if ( Markets::Schema::Result::Category->can('position_column') ) {
