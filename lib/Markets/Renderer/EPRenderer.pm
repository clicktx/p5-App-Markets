...
sub register {
    ...
    $app->renderer->default_handler('ep')->add_handler(
        $conf->{name} || 'ep' => sub {
            ...
            local *{"${ns}::_C"} = sub { $c };
            Markets::Renderer::EPLRenderer::_render( $renderer, $c, $output,
                $options, $mt, $c->stash );
        }
    );
}
...
Markets::Renderer::EPRenderer - Embedded Perl renderer plugin

=head1 SYNOPSIS

  # Mojolicious
  $app->plugin('Markets::Renderer::EPRenderer');
  $app->plugin(Markets::Renderer::EPRenderer => {name => 'foo'});
  $app->plugin(Markets::Renderer::EPRenderer => {name => 'bar', template => {line_start => '.'}});

  # Mojolicious::Lite
  plugin 'Markets::Renderer::EPRenderer';
  plugin Markets::Renderer::EPRenderer => {name => 'foo'};
  plugin Markets::Renderer::EPRenderer => {name => 'bar', template => {line_start => '.'}};

=head1 DESCRIPTION

forked from L<Mojolicious::Plugin::EPRenderer> Mojolicious v6.64

L<Markets::Renderer::EPRenderer> is a renderer for Embedded Perl templates.
