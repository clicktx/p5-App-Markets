...
sub following {
    _select( $_[0]->_collect( @{ $_[0]->_siblings(1)->[1] } ), $_[1] );
}
...
sub preceding {
    _select( $_[0]->_collect( @{ $_[0]->_siblings(1)->[0] } ), $_[1] );
}
...
sub replace {
    ...
    return $self->_replace( $self->_parent, $tree,
        _nodes( $self->_parse($new) ) );
}
...
sub val {
    ...
    return
      exists $self->{multiple} ? $v->size ? $v->to_array : undef : $v->last;
}
...
sub _content {
    ...
    splice @$tree, $start, $offset,
      _link( $tree, _nodes( $self->_parse($new) ) );
...
sub _text {
    ...
    for my $node (@$nodes) {
        ...
        my $chunk = '';
        if ( $type eq 'text' ) {
            $chunk = $trim ? squish $node->[1] : $node->[1];
        }
        ...
        # Nested tag
            ...
            $chunk =
              _text( [ _nodes($node) ], 1, $node->[1] eq 'pre' ? 0 : $trim );
        }
